...
public static class LancamentoValidationRules
{
    ...
    public static IRuleBuilderOptions<T,string> LancamentoCategoriaValida<T>(this IRuleBuilder<T,string> rule) =>
        rule.NotEmpty().Must(LancamentoCategorias.Todas.Contains).WithMessage("Categoria inválida.");
    public static IRuleBuilderOptions<T,string> LancamentoStatusValido<T>(this IRuleBuilder<T,string> rule) =>
        rule.Must(LancamentoStatuses.Todos.Contains).WithMessage("Status inválido.");
