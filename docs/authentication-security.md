...
3. **SQL Server (`sa`)** — a API não usa nem conhece o `sa` (ver "Identidades SQL da API"); só o serviço `sqlserver` e o `sql-bootstrap`. Trocar `SQL_SA_PASSWORD` no `.env` **não** altera a senha já persistida no volume `almirante-sqlserver-data` de um SQL Server existente — é preciso trocar a senha no próprio SQL Server (ex.: `ALTER LOGIN sa WITH PASSWORD = '<nova>'` numa sessão conectada com a senha atual) e só depois atualizar o `.env`. Nunca recrie o volume/banco só para "resolver" a senha sem backup validado.
...
- O nginx não adiciona esses cabeçalhos às respostas da API (evita duplicidade); só às respostas que ele mesmo gera (`429` do login e do refresh; na borda HTTPS também HSTS).
...
## Correções da validação da PR #59

Cada item abaixo tem teste automatizado (`SecurityRegressionTests`, `DbPrivilegeTests`, `scripts/tests/`). Regra central: **`Development` não desliga autenticação, autorização, auditoria nem proteção do banco** — as diferenças entre ambientes ficam em certificados, endereços e diagnóstico.

- **CLI e política TLS.** `Program.cs` executa o `IStartupValidator` (JWT + TLS do SQL Server) **antes** de qualquer acesso ao banco, também para `reset-admin-password`. Um Development *publicado* (IIS/Pop!_OS) liga a mesma exigência (`Encrypt=True`, `TrustServerCertificate=False` nas duas connection strings) com `Security__RequireTrustedSqlServerCertificate=true` (no Compose TLS: `SQL_REQUIRE_TRUSTED_CERTIFICATE=true` no `.env`). **Ordem:** instale/valide o certificado do SQL Server nos clientes (Windows e Linux, com SAN igual ao nome usado na connection string) e só depois ative a flag — ativada antes, a API recusa iniciar.
...
- **Menor privilégio (SQL).** O login da aplicação recebe uma role própria (`almirante_app_role`) com `GRANT` explícito por tabela (SELECT/INSERT/UPDATE; DELETE só em `AuthSessions`; só SELECT em `__EFMigrationsHistory` e `lancamentos_deletados`) — nada no schema inteiro, nenhum papel fixo. Um login já provisionado (ex.: `db_owner`) é normalizado no startup (sai das outras roles, perde permissões diretas). Depois da rotação a API confere as permissões **efetivas** com a própria credencial (`DbPrivilegeAuditor`) e recusa iniciar se sobrar papel de servidor/banco, DDL, DELETE arbitrário, escrita direta na auditoria ou propriedade de objetos — a mensagem lista o achado, sem segredos. A identidade de runtime e a administrativa agora são usuários contidos criados pelo bootstrap (seção "Identidades SQL da API"); a normalização de papéis herdados é do bootstrap, e a API recusa iniciar se sobrar desvio. Para a identidade Windows do IIS (sem `DbCredentials:AppUser`), `Security:DbPrivilegeCheck` = `Warn` (padrão, só registra), `Enforce` ou `Off` — `sa` é recusado em qualquer modo.
...
### Identidades SQL da API: nenhuma delas é `sa`

A API **nunca** executa SQL como `sa`, e isso é imposto por código, não por documentação:

| Identidade | Onde existe | O que pode |
| --- | --- | --- |
| `sa` | só o serviço `sqlserver` (`MSSQL_SA_PASSWORD`) e o serviço `sql-bootstrap` (`SQLCMDPASSWORD`) do Compose | administrar a instância e provisionar as duas identidades abaixo. **Não existe para a API.** |
| Administrativa (`SQL_ADMIN_USER`, padrão `almirante_admin_bd`) | usuário **contido** do banco da aplicação (sem login de servidor) | `CREATE TABLE/VIEW/PROCEDURE/FUNCTION`, `CONTROL` no schema `dbo` e `ALTER ANY USER` — só para migrations, concessões à role de runtime e rotação da senha de runtime. Sem papel/permissão de servidor, sem `db_owner`, sem `CONTROL` no banco, sem acesso a outro banco. |
| Runtime (`SQL_APP_USER`, padrão `almirante_user_bd`) | usuário **contido** do banco, membro só de `almirante_app_role` | `SELECT`/`INSERT`/`UPDATE` nas tabelas de negócio; `DELETE` só em `AuthSessions` (limpeza de sessões); só `SELECT` em `__EFMigrationsHistory` e `lancamentos_deletados` (com `DENY` de escrita: só o trigger grava a auditoria). Sem DDL, `db_owner`, impersonação, `ALTER ANY LOGIN` ou propriedade de objetos. |

Por que **usuários contidos**: um usuário contido é um principal do banco, não do servidor — não pode ser `sysadmin`, ter `CONTROL SERVER`, `ALTER ANY LOGIN`, `CREATE ANY DATABASE` nem `IMPERSONATE ANY LOGIN`, e a rotação da senha vira um `ALTER USER … WITH PASSWORD` restrito ao banco. Com um *login* isso exigiria `ALTER ANY LOGIN`, que permitiria também redefinir a senha de logins de outras aplicações da mesma instância (e assumir a identidade delas). O custo é que a connection string das duas identidades precisa de `Database=<banco>`.

**Quem cria as identidades: o bootstrap, fora da API.** O serviço `sql-bootstrap` do Compose (`scripts/sql-bootstrap.sh` + `docs/sql/criar-usuario-admin-app.sql`) roda uma vez a cada `docker compose up`, antes da API, e é idempotente: liga `contained database authentication` se preciso, cria/atualiza o banco (`CONTAINMENT = PARTIAL`, `TRUSTWORTHY OFF`), cria as identidades, normaliza papéis e permissões herdados e **falha o serviço** (e, portanto, o `up`) se a conferência final não bater. O `sa` chega a ele por variável de ambiente lida pelo `sqlcmd` (`SQLCMDPASSWORD`) e a senha administrativa por `APP_ADMIN_PASSWORD`: nada de senha em linha de comando, em log ou na saída (`scripts/tests/sql-bootstrap.test.sh`). Manualmente, o mesmo script roda com `sqlcmd` (ver o cabeçalho do `.sql`).

**Fail closed (sem modo `Warn`/`Off`):**

- `SqlIdentityPolicy` (validação de startup, antes de qualquer acesso ao banco, também para a CLI `reset-admin-password`): recusa `SQL_SA_PASSWORD`/`MSSQL_SA_PASSWORD` no ambiente da API; `sa` (qualquer caixa, entre colchetes ou com espaços) em **qualquer** connection string ou como `DbCredentials:AppUser`; identidade administrativa ausente (não há fallback), sem credencial, igual à de runtime ou apontando para outro banco; a configuração antiga `Security:AdminPrivilegeCheck` com `Warn`/`Off`.
- `DbPrivilegeAuditor.FindAdminExcessPrivilegesAsync` confere os privilégios **efetivos** da identidade administrativa no SQL Server (não o nome): ser `sa` (nome, SID `0x01`), qualquer papel de servidor perigoso, qualquer permissão de servidor perigosa (`CONTROL SERVER`, `ALTER ANY LOGIN`, `ALTER ANY DATABASE`, `ALTER ANY SERVER ROLE`, `IMPERSONATE ANY LOGIN`…), `IMPERSONATE` em outro principal, papéis fixos do banco (`db_owner`, `db_ddladmin`, `db_securityadmin`, `db_datareader/writer`…), `CONTROL`/`ALTER`/`BACKUP` no escopo do banco, ser dono do banco, banco `TRUSTWORTHY` ou com cadeia de propriedade entre bancos, e qualquer permissão administrativa em **outro** banco. Qualquer achado recusa iniciar **antes** de alterar algo (nenhuma migration, nenhuma credencial publicada).
- A identidade de runtime é auditada de novo com a senha recém-rotacionada (`FindExcessPrivilegesAsync`, incluindo `sa`); desvio (papel a mais, `DELETE` fora de `AuthSessions`, escrita na auditoria, `IMPERSONATE`, propriedade de objetos) recusa iniciar. A API não normaliza papéis (a administrativa não tem `ALTER ANY ROLE`): reaplicar o bootstrap corrige.
- Sem `DbCredentials:AppUser` (ex.: IIS com autenticação do Windows), `sa` continua recusado em qualquer configuração; `Security:DbPrivilegeCheck` só controla o resto.
- Compose: `SQL_ADMIN_USER`/`SQL_ADMIN_PASSWORD` são obrigatórios (`${VAR:?}`, sem padrão), o serviço `api` não recebe o segredo do `sa` (`scripts/tests/compose-sql-identities.test.sh` e `SqlIdentityRepositoryPolicyTests` travam a regressão), o `deploy-config.sh` confere as duas variáveis antes de qualquer alteração de serviço, e o healthcheck do SQL Server não autentica como `sa` (readiness pela resposta "Login failed" a um login inexistente — custo: uma tentativa inválida de `almirante_healthcheck` por intervalo no log do SQL Server).

**Rotação da senha de runtime.** Automática: a API gera 48 caracteres com `RandomNumberGenerator` (~285 bits), troca a senha com `ALTER USER … WITH PASSWORD`, a mantém só em memória e a injeta a cada conexão (`AppDbCredentialInterceptor`). Ocorre em todo startup e a cada `SQL_APP_PASSWORD_ROTATION_HOURS`. Durante a troca, aberturas novas de conexão esperam (`AppDbCredentialProvider.BeginSwitch`) e o pool da credencial anterior é esvaziado, então nenhuma conexão nova nem reaproveitada parte da senha antiga; a senha nunca vai a log, exceção nem argumento de processo (erros do SQL Server são reduzidos a número/estado). Premissa: **uma instância da API por banco** — cada startup rotaciona a senha e invalida a das demais.

**Rotação da credencial administrativa** (sem alterar código; recomendada periodicamente e imediatamente após suspeita de vazamento): (1) gere uma senha nova (`openssl rand -base64 36 | tr -dc 'A-Za-z0-9' | head -c 40`); (2) atualize `SQL_ADMIN_PASSWORD` no `.env`/cofre do ambiente (fora do Git); (3) `docker compose up -d` — o `sql-bootstrap` redefine a senha (`ALTER USER`; nada mais muda) e a API é recriada com a nova; a antiga deixa de valer assim que o bootstrap termina. Se a suspeita for de vazamento, faça também a rotação da senha de runtime (basta reiniciar a API) e revise o log do SQL Server. Para trocar o `SQL_ADMIN_USER`, aplique o bootstrap com o nome novo e remova o usuário antigo (`DROP USER`) no banco.

**Migração de um ambiente anterior** (logins `almirante_admin_bd`/`almirante_user_bd`, `sa` na connection string administrativa): defina `SQL_ADMIN_USER`/`SQL_ADMIN_PASSWORD` no `.env`, rode `docker compose up -d` — o bootstrap converte os logins em usuários contidos (apaga o login de servidor de mesmo nome, recria a identidade e normaliza papéis), muda o banco para `CONTAINMENT = PARTIAL` (janela curta em `SINGLE_USER`: a API antiga perde a conexão) e a API nova sobe. `SQL_SA_PASSWORD` continua necessário só para o SQL Server e o bootstrap. Trocar `SQL_SA_PASSWORD` no `.env` **não** altera a senha já persistida no volume de um SQL Server existente (ver "Rotação de valores expostos").

O banco é criado pelo bootstrap (a identidade administrativa não tem `CREATE DATABASE`, de propósito); se ele não rodou, a API falha com uma mensagem que aponta o bootstrap — nunca cai para `sa`.

**Critério para publicação:** `docker compose up` concluindo com `sql-bootstrap` em `Exited (0)`, API saudável e os testes `Category=RequiresSqlServer` verdes (`SqlIdentityModelTests`, `ApiProcessTests`, `DbPrivilegeTests`). Para uma identidade direta sem `DbCredentials:AppUser` (IIS), exigir também `Security:DbPrivilegeCheck=Enforce`. Rotacionar de fato as senhas e registrar isso no ambiente real continua sendo ação operacional; os testes do CI são descartáveis e não comprovam que ela ocorreu.
...
Agora a classe 4 entra tanto no `REVOKE` do provisionamento quanto em `docs/sql/corrigir-privilegios-usuario-app.sql`, e `DbPrivilegeAuditor` confere `IMPERSONATE` sobre qualquer principal do banco e `EXECUTE` por objeto (procedures/funções). Teste: `DbPrivilegeTests.ImpersonateEmDbo_EDetectadoPelaAuditoria_ERevogadoPeloBootstrap`.
...
5. Garanta configuração e SQL Server compartilhados entre réplicas — exceto a senha de runtime do SQL: com `DbCredentials:AppUser` ela é gerada em memória e cada startup a troca, então rode **uma única instância da API por banco** (uma segunda instância invalidaria a senha da primeira). Antes do primeiro deploy desta versão, defina `SQL_ADMIN_USER`/`SQL_ADMIN_PASSWORD` no `.env` do ambiente (ver "Identidades SQL da API"). Publique backend e frontend coordenadamente: tokens antigos sem `sid` são rejeitados.
