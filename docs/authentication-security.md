...
4. O mesmo `csrfToken` vale para `refresh` e `logout` com ou sem `Authorization`, inclusive com o access token expirado ou a sessão já revogada: a API emite e valida o token sem vínculo com o bearer (`CookieCsrfProtection`), porque a credencial protegida é o cookie. Obtenha outro token só depois de recarregar a página.
